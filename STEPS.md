...
- I have a few simple responses here for a few routes.
- In redwood, every route is just a function. Just using the native Request and Response here. We have max flexibility, and ownership of request and response.
...
- Prisma Hooked up to the cloudflare d1 database! Locally, it uses miniflare to emulate cloudflare workers. It just works between dev and prod. I was working just fine on the plane.
...
- Also using use() to read a promise from the server in this client component and suspend with a fallback. Streaming with Suspense and server components.
- And I also added VT on the suspense.
...
- Exported the realtime durable object in worker.tsx, then wire up our worker route with realtimeRoute of a reactions durable object here, connecting the websocket route to the appropriate durable object.
